...
exports.getStudents = asyncHandler(async (req, res) => {
  ...
  if (req.query.interestedDomain) {
    filter.interestedDomain = {
      $regex: req.query.interestedDomain,
      $options: "i",
    };
  }
  ...
  if (req.query.search) {
    filter.$or = [
      ...
      { interestedDomain: { $regex: req.query.search, $options: "i" } },
    ];
  }
...
exports.createStudent = asyncHandler(async (req, res, next) => {
  ...
  if (!name || !email || !studentId || !password) {
    return next(
      new ApiError("name, email, studentId and password are required", 400),
    );
  }

  const existing = await User.findOne({ $or: [{ email }, { studentId }] });
  if (existing)
    return next(new ApiError("Email or Student ID already exists", 400));

  const cardNum = idCardNumber || admissionNumber || studentId;
...
exports.getStudent = asyncHandler(async (req, res, next) => {
  ...
  const [submissions, interviews] = await Promise.all([
    Submission.find({ user: student._id })
      .populate("quiz", "title category difficulty")
      .sort({ createdAt: -1 }),
    InterviewSession.find({ user: student._id })
      .sort({ createdAt: -1 })
      .limit(5),
  ]);

  res.status(200).json({ success: true, student, submissions, interviews });
});
...
exports.deleteStudent = asyncHandler(async (req, res, next) => {
  ...
  if (student.role === "admin")
    return next(new ApiError("Cannot delete admin", 403));
...
exports.getReports = asyncHandler(async (req, res) => {
  const [submissionTrend, departmentStats, quizPassRate, studentActivity] =
    await Promise.all([
